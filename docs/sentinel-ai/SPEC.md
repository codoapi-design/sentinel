...
# 4. ما الذي يجب أن يفعله Sentinel؟
...
مثلاً: المستخدم لم يسأل عن شيء، ومع ذلك يكتشف Sentinel
...
Sentinel يتعامل مع ثلاثة أنواع من المستخدمين.
...
يعتبر Sentinel ناجحاً عندما يشعر المستخدم بعد كل تفاعل بأنه:
...
> **Normative.** This part is the engineering Source of Truth for Sentinel AI’s Core System Prompt — the laws that govern the agent. Implementation later must follow this specification. No runtime agents, APIs, prompts, or tools are required by this document alone.
...
Sentinel AI is a professional Crypto Portfolio Intelligence Agent.
...
> **Normative (foundation).** This part defines the architecture of Sentinel’s Analysis / Intelligence Framework. It is the product differentiator: the LLM **explains** analysis; it does **not** perform the analysis. Module chapters: **Modules 01–03 recorded** (Performance · Flow · Portfolio); remaining chapters forthcoming (next: **Module 04 — Asset Intelligence**).
...
System Prompt ليس ما يجعل Sentinel ذكياً.

ولا الـ Function Calling.

ولا حتى الـ LLM.

الذي سيجعل Sentinel مختلفاً عن أي منتج آخر هو **Analysis Framework**.
...
لكن Sentinel يجب أن يعمل بطريقة مختلفة تمامًا:
...
```text
Sentinel Intelligence Framework
...
هذا ما يجعل Sentinel أقرب إلى منصة ذكاء مالي حقيقية (Bloomberg-like) وليس مجرد Chat فوق بيانات المحفظة.
...
> **Normative (Module 02).** Flow Intelligence Engine — second chapter of the Sentinel Intelligence Framework. Spec only; no runtime implementation required by this document alone.
...
> **Normative (Module 06).** Trading Intelligence Engine — sixth chapter of the Sentinel Intelligence Framework. Spec only; no runtime implementation required by this document alone.
...
لن يقول Sentinel أبداً: «ادخل هنا» أو «اخرج الآن» أو «هذه صفقة ناجحة».
...
وهذه الوحدة تحديداً هي ما يميز Sentinel عن أدوات التتبع أحادية الشبكة.
...
> **Normative (Module 07).** Network Intelligence Engine — seventh chapter of the Sentinel Intelligence Framework. Spec only; no runtime implementation required by this document alone.
...
> **Normative (Module 08).** Counterparty Intelligence Engine — eighth chapter of the Sentinel Intelligence Framework. Spec only; no runtime implementation required by this document alone.
...
محفظة أخرى **يملكها المستخدم نفسه** داخل Sentinel.
...
وهي الوحدة التي تحدد **متى** يتحدث Sentinel من تلقاء نفسه.
...
المستخدم لا يفتح Sentinel كل يوم؛ لذلك جودة التنبيه هي ما يحدد قيمة المنتج الحقيقية.
...
> **Normative (Module 09).** Alert Intelligence Engine — ninth chapter of the Sentinel Intelligence Framework. Spec only; no runtime implementation required by this document alone.
