...
export default function AdminUsersPage() {
  ...
  const exportCSV = () => {
    const headers = ['البريد', 'الاسم', 'الباقة', 'الحالة', 'المحافظ', 'المعاملات', 'تاريخ التسجيل'];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-4 text-xs text-[#8a8f98]">
        <span>إجمالي المستخدمين: <strong className="text-[#f7f8f8]">{total}</strong></span>
        <span>الصفحة: <strong className="text-[#f7f8f8]">{page}/{totalPages}</strong></span>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/5">
                ...
                <th className="text-right px-4 py-3 text-xs text-[#8a8f98] font-medium">آخر نشاط</th>
                <th className="text-right px-4 py-3 text-xs text-[#8a8f98] font-medium">تاريخ التسجيل</th>
                <th className="text-center px-4 py-3 text-xs text-[#8a8f98] font-medium">إجراءات</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="text-center py-12 text-[#8a8f98]">لا يوجد مستخدمين</td>
                </tr>
              ) : (
                users.map((user) => (
                  <tr key={user.user_id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          <p className="text-[#f7f8f8] truncate text-xs font-medium">{user.full_name || 'بدون اسم'}</p>
                    ...
                    <td className="px-4 py-3 text-xs text-[#8a8f98]">
                      {user.last_active_at ? new Date(user.last_active_at).toLocaleDateString('ar') : '—'}
                    </td>
                    <td className="px-4 py-3 text-xs text-[#8a8f98]">
                    ...
                    <td className="px-4 py-3">
                      <div className="relative flex justify-center">
                        ...
                        {actionMenuOpen === user.user_id && (
                          <div className="absolute top-full left-0 mt-1 w-44 bg-[#191a1b] border border-white/10 rounded-xl shadow-xl z-20 py-1.5">
                            <button
                            ...
                            {user.status === 'active' && (
                              ...
                              >
                                <Ban className="h-3.5 w-3.5" /> تعليق الحساب
                              </button>
                            )}
                            ...
                            {user.status === 'suspended' && (
                              ...
                              >
                                <CheckCircle className="h-3.5 w-3.5" /> تفعيل الحساب
                              </button>
                            )}
                            ...
                            {user.status !== 'banned' ? (
                              ...
                              >
                                <XCircle className="h-3.5 w-3.5" /> حظر المستخدم
                              </button>
                            ) : (
                              <button
                            ...
                            <div className="border-t border-white/5 mt-1.5 pt-1.5">
                              <p className="px-3 py-1 text-[10px] text-[#8a8f98]">تغيير الباقة</p>
